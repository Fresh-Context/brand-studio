...
const SRC = path.join(dirs().library, 'brand-guideline', 'whoweare.html');
...
fs.writeFileSync(OUT_HTML, `<style>\n${scopedCss}\n</style>\n<div class="identity-view">\n${body.trim()}\n</div>\n`);
fs.writeFileSync(OUT_JS, js.trim() + '\n');

console.log(`identity-embed.html  ${(fs.statSync(OUT_HTML).size / 1024 / 1024).toFixed(2)}MB`);
console.log(`identity-embed.js    ${fs.statSync(OUT_JS).size} bytes`);
